...
def ar5_postprocess_icesheets(
    ...
    gissamps = project_dict["gissamps"]
    waissamps = project_dict["waissamps"]
    eaissamps = project_dict["eaissamps"]
    ...
    gisfp = da.array(
        AssignFP(os.path.join(fingerprint_dir, "fprint_gis.nc"), site_lats, site_lons)
    )
    waisfp = da.array(
        AssignFP(os.path.join(fingerprint_dir, "fprint_wais.nc"), site_lats, site_lons)
    )
    eaisfp = da.array(
        AssignFP(os.path.join(fingerprint_dir, "fprint_eais.nc"), site_lats, site_lons)
    )
    ...
    ncvar_attributes = {
        "description": "Local SLR contributions from icesheets according to AR5 workflow",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the post-processing stage for the AR5 icesheets SLR projection workflow",
